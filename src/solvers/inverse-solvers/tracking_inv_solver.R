...
TrackingOdeSolver <- R6Class("TrackingOdeSolver",
  ...
  private = list(
    ...
    outer_gradient_adjoint = function(theta_norm, param_names) {
  ...
  public = list(
    ...
    # Outer Objective: H(theta) = J(y*(theta), u*(theta); theta)
    ...
    outer_gradient = function(theta_norm, param_names, gradient_mode = "adjoint") {
      mode <- match.arg(gradient_mode, c("adjoint", "sensitivity"))
      if (mode == "adjoint") {
        return(private$outer_gradient_adjoint(theta_norm, param_names))
      }
      if (mode == "sensitivity"){
        return(private$outer_gradient_sensitivity(theta_norm, param_names))
      }
    },
    ...
    optimize_parameters = function(init_theta_physical, param_names,
      ...
      res <- optim(
        ...
        gr          = function(par, param_names) {
          self$outer_gradient(par, param_names, gradient_mode = gradient_mode)
        },
        param_names = param_names,
        method      = "L-BFGS-B",
        lower       = lower_norm,
